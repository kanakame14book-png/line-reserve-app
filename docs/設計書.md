...
| `/admin/checkin?id=` | 受付票URLからの受付 | スタッフ | Supabase Auth |
...
| `受付済` | 当日、QR受付を完了。 | scanner / checkin での受付 |
...
> **重要**：`登録` のみの状態では当日受付はできない（scanner / checkin が受付を拒否する）。受付には `予約` への移行が必要。
...
- **QRコード生成を外部API（`api.qrserver.com`）に依存**。可用性・個人情報の観点で見直し余地あり。
